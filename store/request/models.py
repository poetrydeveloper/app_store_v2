...
from goods.models import Product  # Импорт из приложения product
...
class RequestItem(models.Model):
    ...
    def clean(self):
        super().clean()
        # Проверка цены (разрешена 0 только при создании)
        if not self._state.adding and self.price_per_unit <= 0:
            raise ValidationError({
                'price_per_unit': _('Цена должна быть больше 0.')
            })

        # Проверка количества
        if self.quantity < 1:
            raise ValidationError({
                'quantity': _('Количество не может быть меньше 1.')
            })

    def save(self, *args, **kwargs):
        # Установка цены 0 для новых позиций если не указана
